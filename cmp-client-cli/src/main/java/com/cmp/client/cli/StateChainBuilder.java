...
public static java.util.List<byte[]> buildAndWriteChain(StateTableApi api, String sid, String ds, byte[] alpha, byte[] st0, int count){
    java.util.List<byte[]> states = new java.util.ArrayList<>();
    byte[] st = st0;
    states.add(st0);
    for (int t=0; t<count; t++){
        byte[] stNext = new byte[st.length];
        new java.security.SecureRandom().nextBytes(stNext);
        byte[] mask = com.cmp.core.crypto.HashUtil.H2(com.cmp.core.util.Bytes.concat(
                sid.getBytes(java.nio.charset.StandardCharsets.UTF_8),
                new byte[]{0x00},
                ds.getBytes(java.nio.charset.StandardCharsets.UTF_8),
                new byte[]{0x00},
        ...
        byte[] sk = new byte[st.length];
        for (int i=0;i<st.length;i++) sk[i] = (byte)(stNext[i] ^ mask[i]);
        byte[] h1st = com.cmp.core.crypto.HashUtil.H1(st);
        api.put(new com.cmp.protocol.dto.StateTableDTO.PutRequest(sid, ds, t, h1st, sk));
        st = stNext;
        states.add(st);
    }
    byte[] h1Last = com.cmp.core.crypto.HashUtil.H1(st);
    api.put(new com.cmp.protocol.dto.StateTableDTO.PutRequest(sid, ds, count, h1Last, null)); // 断点
